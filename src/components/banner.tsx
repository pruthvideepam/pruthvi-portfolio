...
const Banner = ({ hide, setHide }: BannerProps) => {
  ...
  className={`${
    hide ? "hidden" : "block"
  } fixed h-20 top-0 left-0 z-50 flex justify-between w-full p-4 bg-primary shadow-md opacity-[0.98]`}
>
      <div className="flex items-center mx-auto">
        <p className="flex items-center text-sm font-normal text-gray-500">
          ...
          <span className="text-white">
            Careful: scrolling further may cause sudden urges to refactor your
            own codebase;)
          </span>
        </p>
      </div>
